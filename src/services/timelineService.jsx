export const getAllTimelines = (userId) => {
  return fetch(
    `http://localhost:3000/timelines?userId=${userId}&_expand=category`
  ).then((response) => response.json());
};

export const getTimelineById = (id) => {
  return fetch(`http://localhost:3000/timelines/${id}?_expand=category`).then(
